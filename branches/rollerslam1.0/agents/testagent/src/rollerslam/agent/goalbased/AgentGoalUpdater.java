	...
	public void updateGoal(GoalBasedEnvironmentStateModel goal) {
		...
		if (state == SimulationState.STOPPED) {
			...
			if (model.gameStarted) {
				...
				try {
					GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
							me.world.currentCycle, me.id, "Go to Goal");
			...
			Player me = model.getMe();
			if (!me.inGround) {
				if (MathGeometry.calculeDistancePoints(me.s.x,
						me.world.ball.s.x, me.s.y, me.world.ball.s.y) < 5000) {
					if (!me.world.ball.withPlayer()) {
						try {
							GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
									me.world.currentCycle, me.id,
									"Catch the ball");
							facade.getLogRecordingService().addEntry(envLog);
						} catch (RemoteException e) {
							if (PrintTrace.TracePrint) {
						...
						if (me.hasBall()) {
							...
							if (me.world.playerWithBall.team == me.team) {
								try {
									GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
											me.world.currentCycle, me.id,
											"Ball with team. Do nothing");
									facade.getLogRecordingService().addEntry(
											envLog);
								} catch (RemoteException e) {
									if (PrintTrace.TracePrint) {
										e.printStackTrace();
				...
				try {
					GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
							me.world.currentCycle, me.id, "Stand Up");
					facade.getLogRecordingService().addEntry(envLog);
				} catch (RemoteException e) {
					if (PrintTrace.TracePrint) {
						e.printStackTrace();
			...
			if (me.hasBall()) {
				...
				if (me.team == PlayerTeam.TEAM_A) {
					for (Player p : me.world.playersB) {
						if (MathGeometry.calculeDistancePoints(me.s.x, p.s.x,
								me.s.y, p.s.y) < 5000
								&& Math.random() > 0.5) {
							nearOpponent = true;
						}
				...
				if (nearOpponent) {
					try {
						GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
								me.world.currentCycle, me.id, "CounterTackle");
						facade.getLogRecordingService().addEntry(envLog);
					} catch (RemoteException e) {
						if (PrintTrace.TracePrint) {
							e.printStackTrace();
				...
				if (me.world.playerWithBall != null
						&& me.world.playerWithBall.team == me.team) {
					try {
						GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
								me.world.currentCycle, me.id,
								"Ball with team. Do nothing");
					...
					try {
						GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
								me.world.currentCycle, me.id, "Go to ball");
						facade.getLogRecordingService().addEntry(envLog);
					} catch (RemoteException e) {
						if (PrintTrace.TracePrint) {
							e.printStackTrace();
			...
			if (me.inGround) {
				...
				if (me.team == PlayerTeam.TEAM_A) {
					if (MathGeometry.calculeDistancePoints(me.s.x,
							me.world.goalB.s.x, me.s.y, me.world.goalB.s.y) < 30000) {
						try {
							GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
									me.world.currentCycle, me.id,
									"Throw the ball");
					...
					} else if (MathGeometry.calculeDistancePoints(me.s.x,
							me.world.goalB.s.x, me.s.y, me.world.goalB.s.y) < 50000) {
						try {
							GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
									me.world.currentCycle, me.id,
									"Kick the ball");
					...
					if (MathGeometry.calculeDistancePoints(me.s.x,
							me.world.goalA.s.x, me.s.y, me.world.goalA.s.y) < 30000) {
						try {
							GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
									me.world.currentCycle, me.id,
									"Throw the ball");
					...
					if (MathGeometry.calculeDistancePoints(me.s.x,
							me.world.goalA.s.x, me.s.y, me.world.goalA.s.y) < 50000) {
						try {
							GoalUpdateLogEntry envLog = new GoalUpdateLogEntry(
									me.world.currentCycle, me.id,
									"Kick the ball");
