...
public class MobileCanvas extends GameCanvas {
	...
	public void paint(Graphics g) {
		...
		if (model != null && model.getSimulationState() != null
				&& model.getSimulationState().getData() != null) {
			...
			teamAScore = worldView[worldView.length - 2];
			teamBScore = worldView[worldView.length - 1];

			g.setColor(0);
			g.drawString("A " + teamAScore, 0, 0, Graphics.TOP | Graphics.LEFT);

			g.setColor(0);
			g.drawString(teamBScore + " B", getWidth(), 0, Graphics.TOP
					| Graphics.RIGHT);
		}
	}
	...
	private void drawThing(Graphics g, int color, int wx, int wy) {
		...
		g.setColor(color);
		g.fillArc(ox - THING_SIZE / 2, oy - THING_SIZE / 2, THING_SIZE,
				THING_SIZE, 0, 360);

		g.setColor(0);
		g.drawArc(ox - THING_SIZE / 2, oy - THING_SIZE / 2, THING_SIZE,
				THING_SIZE, 0, 360);
	}
