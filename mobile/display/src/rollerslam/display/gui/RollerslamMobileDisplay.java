...
public class RollerslamMobileDisplay {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (host == null || "AUTO".equalsIgnoreCase(host)) {
            facade.getClientInitialization().init();
        } else {
            facade.getClientInitialization().init(host);
        }
        facade.getDisplayRegistry().register((Display) facade.getClientInitialization().exportObject(new Display() {
        ...
        while (true) {
            //System.out.println("Simulation host");
    ...
    private static String getHost(String[] args) {
        if (args.length == 0) {
            return JOptionPane.showInputDialog("Simulation host", "AUTO");
        } else {
            return args[0];
        }
    }
