...
poss(release(P),Z):-holds(P@[hasBall->true],Z).
...
poss(spike(P, vector(Sbx, Sby)),Z):- holds(P@[position->vector(Sx, Sy)], Z),
                   closer(Sx, Sy, Sbx, Sby, 500).

poss(volley(P,vector(Sbx, Sby)),Z):- holds(P@[position->vector(Sx, Sy)], Z),
                    closer(Sx, Sy, Sbx, Sby, 500).
